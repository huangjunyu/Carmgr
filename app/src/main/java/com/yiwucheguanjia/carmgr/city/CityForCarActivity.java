 ...
 * 城市选择的主类
 */
public class CityForCarActivity extends Activity {
...
//    private ClearEditText clearEditText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cityforcar);
        initData();
        initViews();
        setListener();
    }
    ...
    private void initViews() {
        ...
        gobackRl = (RelativeLayout) findViewById(R.id.cityforcar_goback_rl);
        sideBar.setTextView(dialog);
        sortListView = (ListView) findViewById(R.id.country_lvcountry);
        sortListView.addHeaderView(view);
        adapter = new SortAdapter(this, SourceDateList);
        sortListView.setAdapter(adapter);
        gobackRl.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void setListener() {
        ...
        sideBar.setOnTouchingLetterChangedListener(new SideBar.OnTouchingLetterChangedListener() {
            ...
            public void onTouchingLetterChanged(String s) {
                KeyBoard.closeSoftKeyboard(CityForCarActivity.this);
                //该字母首次出现的位置
        ...
        sortListView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (cityName != null && cityName.length() > 0) {
                    Toast.makeText(CityForCarActivity.this, cityName, Toast.LENGTH_SHORT).show();
                    KeyBoard.closeSoftKeyboard(CityForCarActivity.this);
//                    setResult(CAR_RESULT_CITY_CODE, new Intent().putExtra("carRequestCity", cityName));
//                    finish();

                    Intent cityIntent = new Intent();
                    cityIntent.putExtra("carRequestCity", cityName);
                    cityIntent.putExtra("provinceNum",((CityModel) adapter.getItem(position - 1)).getParent());
                    Log.e("getparent",((CityModel) adapter.getItem(position - 1)).getParent() + "");
//                    sortListView.getId();
                    setResult(CAR_RESULT_CITY_CODE, cityIntent);
                    finish();
                }
            }
        });
...
//        });
        ...
        gridView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                if (cityName != null && cityName.length() > 0) {
                    ...
                    Intent cityIntent = new Intent();
                    cityIntent.putExtra("carRequestCity", cityName);
                    cityIntent.putExtra("provinceNum",hotCity.get(position).getParent());
                    setResult(CAR_RESULT_CITY_CODE, cityIntent);
                    finish();
                }
            }
    ...
    private List<CityModel> filledData(List<RegionInfo> date) {
        ...
        for (int i = 0; i < date.size(); i++) {
            ...
            cityModel.setParent(date.get(i).getParent());
            ...
            if (provinceName.contains(keyword)) {
                ...
                if (filterDateList.size() == 0) {
                    ...
                    if (filterDateList.size() == 0) {
                        ...
                        Toast.makeText(CityForCarActivity.this, "您输入的关键字有误，请重新输入！", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        }
    ...
    public void searchKey(String keyword) {
        ...
        if (TextUtils.isEmpty(keyword)) {
            ...
            if (filterDateList.size() > 0) {
                ...
                SharedPreferencesUtils.clearData(CityForCarActivity.this);
                SharedPreferencesUtils.saveAreaName(CityForCarActivity.this, "全部", 0);
                for (int i = 1; i < filterDateList.size(); i++) {
                    Log.e("filterDate", filterDateList.get(i).getName() + i);
                    SharedPreferencesUtils.saveAreaName(CityForCarActivity.this, filterDateList.get(i).getName(), i);
                }
            }
        }
